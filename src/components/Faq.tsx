...
const faqs = [
  ...
  const Faq: React.FC = () => {
    return (
      <div className="max-w-[1300px] px-6 sm:px-10 mx-auto pt-24" id='faq'>
    ...
    return (
      <div className="w-full border-b border-gray-300 pb-3 sm:pb-4 my-6 cursor-pointer " onClick={() => setIsOpen(!isOpen)}>
        <div className="flex justify-between items-center">
          <p className="font-medium text-sm sm:text-lg  pr-2">{question}</p>
          <span className={`text-blue ${isOpen ? "rotate-180" : ""} transition-all`}>
            <FaChevronDown />
          </span>
        </div>
        {isOpen && <div className="mt-4 leading-relaxed text-sm sm:text-lg">{answer}</div>}
      </div>
    );
  };